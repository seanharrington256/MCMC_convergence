...
storage_IG<-"~/diags_IG/summaries"
storage_NewHeat<-"~/diags_New_heat/Summaries"
storage_NstMixed<-"~/diags_NstMixed/Summaries"
# Set up the location of the original  burnin and diagnostics, etc.
orig_storage<-"~/Orig_diags"
...
diagnostics<-out_all_IG[which(!lapply(out_all_IG, function(x) x[[1]])=="FAILED")]
## Name the diagnostics by analysis
analysis_names<-lapply(diagnostics, function(x) gsub("\\.nex|\\.run1.t.*|_processed_mbReady", "", names(x$Burnin[1])))
...
## Pars fail ESS
failed_ESS_orig<-orig_IG_diags_LARGE[[3]] #pars fail ESS for original diags
failed_ESS_num_orig<-sapply(failed_ESS_orig, length)# get numnber of pars that originally failed ESS
## Raw diags
raw_diags_orig<-orig_IG_diags_LARGE[[4]] #all raw original diags
...
setwd(write_to)
write.csv(diffs_reanalysis, file="Diffs_IG_large.csv")
...
G_only_burns<-lapply(diagnostics, function(x) max(unlist(x$Burnin)))
names(G_only_burns)<-lapply(diagnostics, function(x) gsub("\\.run.*$|\\.nex.*$|_processed_mbReady.*$", "", names(x$Burnin)[[1]]))
save(G_only_burns, file="G_only_burns.RData")
...
diagnostics<-diagnostics[which(names(diagnostics) %in% c(bad_ESS, bad_topo_bad_LnL, bad_topo_good_LnL))]
...
# ## Fail Geweke's
# bad_gew<-read.csv("fail_Geweke_pass_common_LARGE.csv", stringsAsFactors = FALSE, row.names=1)[[1]]
# # Pull out the analyses that match these
# indices_bad_gew<-unlist(lapply(bad_gew, function(x) which(gsub("\\.nex.*$|\\.run.*$|_processed_mbReady", "",rownames(pass_fail_combined))==x)))
# prop_pass_bad_gew<-apply(pass_fail_combined[indices_bad_gew,], 2, function(x) length(which(x==TRUE))/length(x))
...
num_failed_diags_orig<-apply(pass_fail_reduced_orig, 1, function(x) length(which(x==FALSE)))
## Pars fail ESS
failed_ESS_orig<-orig_diags_LARGE[[3]] #pars fail ESS for original diags
failed_ESS_num_orig<-sapply(failed_ESS_orig, length)# get numnber of pars that originally failed ESS
## Raw diags
raw_diags_orig<-orig_diags_LARGE[[4]] #all raw original diags
...
## add in a column that specifies which of the subset analyses each of these was from
# diffs_reanalysis_heat[which(diffs_reanalysis_heat[,"analysis_base_names"] %in% bad_gew), 4]<-sapply(diffs_reanalysis_heat[which(diffs_reanalysis_heat[,"analysis_base_names"] %in% bad_gew), 4], function(x) paste(x, "bad_gew", sep=""))
...
## Get the mean, median, and standard deviation for each of the subsets
# Means
# summ_diffs_bad_gew<-apply(diffs_reanalysis_heat[which(diffs_reanalysis_heat[,"analysis_base_names"] %in% bad_gew), 2:3], 2, function(x) mean(as.numeric(x)))
...
summ_diffs<-rbind(summ_diffs_bad_ESS, summ_diffs_bad_topo_bad_LnL, summ_diffs_bad_topo_good_LnL)
colnames(summ_diffs)<-c("Failed diags", "failed ESS")
rownames(summ_diffs)<-c("fail ESS", "fail topo ESS fail LnL ESS", "fail topo ESS pass Lnl ESS")
write.csv(summ_diffs, file="Mean_diffs_summ_heat.csv")
# Medians
# summ_diffs_bad_gew_med<-apply(diffs_reanalysis_heat[which(diffs_reanalysis_heat[,"analysis_base_names"] %in% bad_gew), 2:3], 2, function(x) median(as.numeric(x)))
...
summ_diffs_med<-rbind(summ_diffs_bad_ESS_med, summ_diffs_bad_topo_bad_LnL_med, summ_diffs_bad_topo_good_LnL_med)
colnames(summ_diffs_med)<-c("Failed diags", "failed ESS")
rownames(summ_diffs_med)<-c("fail ESS", "fail topo ESS fail LnL ESS", "fail topo ESS pass Lnl ESS")
write.csv(summ_diffs_med, file="Median_diffs_summ_heat.csv")
# Std Deviation
# summ_diffs_bad_gew_sd<-apply(diffs_reanalysis_heat[which(diffs_reanalysis_heat[,"analysis_base_names"] %in% bad_gew), 2:3], 2, function(x) sd(as.numeric(x)))
...
summ_diffs_sd<-rbind(summ_diffs_bad_ESS_sd, summ_diffs_bad_topo_bad_LnL_sd, summ_diffs_bad_topo_good_LnL_sd)
colnames(summ_diffs_sd)<-c("Failed diags", "failed ESS")
rownames(summ_diffs_sd)<-c("fail ESS", "fail topo ESS fail LnL ESS", "fail topo ESS pass Lnl ESS")
write.csv(summ_diffs_sd, file="StdDev_diffs_summ_heat.csv")

# Get the burnin for each chain for later
NewHeat_burns<-lapply(diagnostics, function(x) max(unlist(x$Burnin)))
names(NewHeat_burns)<-lapply(diagnostics, function(x) gsub("\\.run.*$|\\.nex.*$|_processed_mbReady.*$", "", names(x$Burnin)[[1]]))
save(NewHeat_burns, file="NewHeat_burns.RData")
...
## add in a column that specifies which of the subset analyses each of these was from
# diffs_reanalysis_nstMixed[which(diffs_reanalysis_nstMixed[,"analysis_base_names"] %in% bad_gew), 4]<-sapply(diffs_reanalysis_nstMixed[which(diffs_reanalysis_nstMixed[,"analysis_base_names"] %in% bad_gew), 4], function(x) paste(x, "bad_gew", sep=""))
...
## Get the mean, median, and standard deviation for each of the subsets
# Means
# summ_diffs_bad_gew<-apply(diffs_reanalysis_nstMixed[which(diffs_reanalysis_nstMixed[,"analysis_base_names"] %in% bad_gew), 2:3], 2, function(x) mean(as.numeric(x)))
...
# Medians
# summ_diffs_bad_gew_med<-apply(diffs_reanalysis_nstMixed[which(diffs_reanalysis_nstMixed[,"analysis_base_names"] %in% bad_gew), 2:3], 2, function(x) median(as.numeric(x)))
...
# Std Deviation
# summ_diffs_bad_gew_sd<-apply(diffs_reanalysis_nstMixed[which(diffs_reanalysis_nstMixed[,"analysis_base_names"] %in% bad_gew), 2:3], 2, function(x) sd(as.numeric(x)))
...
# Get the burnin for each chain for later
nstMixed_burns<-lapply(diagnostics, function(x) max(unlist(x$Burnin)))
names(nstMixed_burns)<-lapply(diagnostics, function(x) gsub("\\.run.*$|\\.nex.*$|_processed_mbReady.*$", "", names(x$Burnin)[[1]]))
save(nstMixed_burns, file="nstMixed_burns.RData")
