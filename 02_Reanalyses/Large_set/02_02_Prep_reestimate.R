...
main_dir<-getwd()
chains_dir<-"~/all_original_analyses"  # where folders housing the files from the original chains are
processed_dir<-"~/Prepped_Nex" # where we want to write the processed nexus files - this has to be in the full notation that terminal uses, rather than R, because of the way that this will be called
mb_ready_dir<-"~/Nex_new_heat_chains_READY"  # directory for files that are ready for MrBayes with the new heating
mb_ready_dir_nstMixed<-"~/Nex_nstMixed_chains_READY"  # directory for files that are ready for MrBayes with nst=mixed
## For IG analyses
chains_dir_IG<-"~/IG_original_analyses"  # where folders housing the files from the original chains are
processed_dir_IG<-"~/Prepped_Nex_IG" # where we want to write the processed nexus files - this has to be in the full notation that terminal uses, rather than R, because of the way that this will be called
mb_ready_dir_IG<-"~/Nex_IG_chains_READY"  # directory for files that are ready for MrBayes with the new heating or number of chains
...
for(i in 1:length(folders)){
  ...
  slurm<-c("#!/bin/bash", "", paste("#SBATCH -J", folders[[i]], sep=" "), "#SBATCH -n 8",  # Set up an object with all of the slurm info
           "#SBATCH -c 1", "#SBATCH -N 1", "#SBATCH -p community.q", "#SBATCH --mem-per-cpu 6400MB",
           "#SBATCH -t 3-00:00", paste("#SBATCH -o ",folders[[i]], ".out", sep=""),
           paste("#SBATCH -e ", folders[[i]], ".err", sep=""), "source ~/.bash_profile",
           "module load bioinfo/mrbayes/3.2.5-mpi", paste("mpirun -np 8 mb ", nexus_name, sep=" "))
