## Script to compare the tree topologies for the datasets were reanalyzed manually

library(phangorn)
library(rwty)

# Set up the main directory with the scripts and the directory with the folders to be iterated over
main_dir<-"~/Compare_diags_topo_manual"
storage_old<-"~/Manual_Reanalyses/00_Orig_chains"
storage_HKY<-"~/Manual_Reanalyses/01_Reanalyses/HKY/Data"
storage_empbfreq<-"~/Manual_Reanalyses/01_Reanalyses/emp_base_freq/Data"
storage_run_longer<-"~/Manual_Reanalyses/01_Reanalyses/Run_longer/Data"
out_dir<-"~/Manual_Reanalyses/04_Plots_etc"
...
max_gens<-10000000  # we want all chains to be read in up to the 10,000,000th generation so that all are the same
        ### ^ however, we will change this lower for the analyses that we run longer to allow there to be more samples in these analyses
...
old_to_read<-unique(c(folders_HKY, folders_empbfreq, folders_run_longer))
...
### Load in the original burnins
  load("orig_manual_burns.RData")
  
  
  ## New burn ins
  load("HKY_burnin.RData")
  load("EmpBFreq_burnin.RData")
  load("Run_longer_burnin.RData")
  load("Run_longerMoreSample_burnin.RData")
...
HKY_RF<-compare_rf(reanalyses = chains_HKY, originals = chains_old, new_burn = hky_burns, old_burn = manual_burns)

## RF distances pre and post empirical base frequency
empbfreq_RF<-compare_rf(reanalyses = chains_empbfreq, originals = chains_old, new_burn = emp_BF_burns, old_burn = manual_burns)

### RF distances comparing run longer
run_longer_RF<-compare_rf(reanalyses = chains_run_longer, originals = chains_old, new_burn = Run_longer_burns, old_burn = manual_burns)

### RF distances comparing run longer with more samples
run_longerMore_RF<-compare_rf(reanalyses = chains_run_longer_more, originals = chains_old, new_burn = Run_longerMore_burns, old_burn = manual_burns)



rf_comparisons<-list(HKY_RF, empbfreq_RF, run_longer_RF, run_longerMore_RF)
names(rf_comparisons)<-c("HKY_RF", "empbfreq_RF", "run_longer_RF", "run_longerMore_RF")
...
run_longer_RF<-rf_comparisons[[3]]
run_longerMore_RF<-rf_comparisons[[4]]
...
run_longer_RF_only<-do.call(rbind, lapply(run_longer_RF, function(x) x[[1]]))
...
setwd(out_dir)
pdf(file="RF manual.pdf", height=8, width=11)
par(mfrow=c(4,3))
# HKY
...
hist(HKY_RF_only[,3], col="gray", main=paste("HKY MCC \nMedian = ", round(median(HKY_RF_only[,3]), 3),sep=""), xlab=NULL, xlim=c(0,1), ylab=NULL, breaks=6)
# EmpBFreq
...
# Run Longer
hist(run_longer_RF_only[,1], col="gray", main=paste("Run longer 95% con \nMedian = ", round(median(run_longer_RF_only[,1]), 3), sep=""), xlab="Norm RF", xlim=c(0,1), ylab=NULL)
hist(run_longer_RF_only[,2], col="gray", main=paste("Run longer 50% con \nMedian = ", round(median(run_longer_RF_only[,2]), 3), sep=""), xlab="Norm RF", xlim=c(0,1), ylab=NULL)
hist(run_longer_RF_only[,3], col="gray", main=paste("Run longer MCC \nMedian = ", round(median(run_longer_RF_only[,3]), 3), sep=""), xlab="Norm RF", xlim=c(0,1), ylab=NULL)
