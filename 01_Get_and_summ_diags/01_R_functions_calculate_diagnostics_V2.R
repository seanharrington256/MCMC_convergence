...
# Get the burnin of a chain automatically
...
##  calculates ESS and autocorrelation time for a parameter through an MCMC - this is used interanlly in the below function param_vals_ess
...
param_vals_ess<-function (chains, burnin = 0, cutoff=100, parameter = "LnL") {
  ...
  ptable = combine.ptables(chains, burnin) # note that this burnin is in number of generations, not a percentage - not problematic because we don't use these values for anything
...
## Get only parameter values - burnin here is number of iterations
...
final_ess_ACT_all<-function(chains, burn){
  ...
  res<-list(ess_act=ess_act_only, par_vals=par_vals_only)
  return(res)
}
...
## Modify the internal rwty tree.ess function to use the modified tracerer function calc.ess.act for calculating ESS and autocorrelation time
...
### this function gets the average standard deviation and a table of the clade frequencies from MCMC chains
###   it is a modified version of rwty:::get.comparison.table
###   if there are 4 or more chains, get asdsf for 4 chains
###   if there are fewer than 4 chains, get asdsf for 2 chains
get.compTable.asdsf<-function (chains, burnin, min.freq){
  setnames = names(chains)
  clade.table <- clade.freq(chains[[1]], start = burnin, end = length(chains[[1]]$trees))
  if (is.na(setnames[1])) {
    colnames(clade.table)[2] <- paste("set", 1, sep = ".")
  }
  else {
    colnames(clade.table)[2] <- setnames[1]
  }
  for (i in 2:length(chains)) {
    thistable <- clade.freq(chains[[i]]$trees, start = burnin, 
                            end = length(chains[[i]]$trees))
    clade.table <- merge(clade.table, thistable, by = "cladenames", 
                         all = TRUE)
    if (is.na(setnames[i])) {
      colnames(clade.table)[i + 1] <- paste("set", i, sep = ".")
    }
    else {
      colnames(clade.table)[i + 1] <- setnames[i]
    }
  }
  clade.table[is.na(clade.table)] <- 0

  ## modifications here to the rwty function - instead of spitting out asdsf for pairwise comparisons of
  ##   chains, spit out asdsf of all chains
  asdsf.clade.table <- clade.table[, !colnames(clade.table) %in% "cladenames"]
  asdsf.clade.table<-asdsf.clade.table[apply(asdsf.clade.table, MARGIN = 1, function(x) any(x > min.freq)),]
  
  ## If there are 4 or more chains, get asdsf across 4 chains and across 2 chains if not, get asdsf across 2 chains
  if(length(chains)>=4){
    # for 4 chains
    sd<-apply(asdsf.clade.table[,1:4], 1, sd, na.rm = TRUE)
    asdsf<-mean(sd)
    maxsdsf<-max(sd)
    # for 2 chains
    sd2chains<-apply(asdsf.clade.table[,1:2], 1, sd, na.rm = TRUE)
    asdsf2chains<-mean(sd2chains)
    maxsdsf2chains<-max(sd2chains)
    asdsf_out<-list(asdsf=asdsf, asdsf2chains=asdsf2chains)
    maxsdsf_out<-list(maxsdsf=maxsdsf, maxsdsf2chains=maxsdsf2chains)
  }else{
    sd<-apply(asdsf.clade.table[,1:2], 1, sd, na.rm = TRUE)
    asdsf<-mean(sd)
    maxsdsf<-max(sd)
    asdsf_out<-asdsf
    maxsdsf_out<-maxsdsf
  }
  thissd <- apply(clade.table[, 2:length(clade.table[1, ])], 
                  1, sd)
  thismean <- apply(clade.table[, 2:length(clade.table[1, ])], 
                    1, mean)
  clade.table$sd <- thissd
  clade.table$mean <- thismean
  clade.table <- clade.table[order(clade.table$sd, decreasing = TRUE), 
                             ]
  translation.table <- cbind(as.numeric(clade.table[, 1]), 
                             as.character(clade.table[, 1]), rwty:::parse.clades(clade.table[, 
                                                                                             1], chains[[1]]))
  clade.table[, 1] <- as.numeric(clade.table[, 1])
  output <- list(cladetable = clade.table, asdsf = asdsf_out, maxsdsf=maxsdsf_out, asdsf.min.freq = min.freq, 
...
## Function to get the correlations in split frequencies among chains and ASDSF
## takes a few pieces from makeplot.splitfreq.matrix and from https://stackoverflow.com/questions/45825685/correlations-for-pairs-of-combinations
## and uses get.compTable.asdsf from directly above
##   these are multi-chain diags - requires multiple chains
###   if there are 4 or more chains, get asdsf & split frequency correlations for 4 chains
###   if there are fewer than 4 chains, get asdsf & split frequency correlations for 4 chains
get.sf.cors.asdsf<-function (chains, burn = 0){
  chains<-check.chains(chains)
  burnin = (ceiling((burn*0.01*(length(chains[[1]]$trees)-1)))+1)
  dat = get.compTable.asdsf(chains, burnin, min.freq = 0.1)
  asdsf = dat$asdsf
  dat = dat$cladetable
  ...
  ## get out average of split frequency correlations of 6 pairwise comparisons (corresponding to 4 chains) if there are that many or more
  ##   as well as correlation among just 2 chains for comparison
  if(nrow(cr_data_frame)>=4){
    avg_cor_freq<-mean(cr_data_frame[1:6,"cor"])
    cor_freq_2chains<-cr_data_frame[1,"cor"]
    sf_cors<-list(avg_cor_freq=avg_cor_freq, cor_freq_2chains=cor_freq_2chains)
  }else{
    sf_cors<-cr_data_frame[1,"cor"]
  }
  res<-list(sf_corrs=sf_cors, ASDSF=asdsf)
  return(res)
}
...
# Trim is the thinning interval to be passed to load.multi, target_samp is the number of total samples to shoot for at the end (if there are that many), and max_gens is last generation that we want to include - e.g., if we want to read in only the first 100,000 generations of an analysis, we set this to 100,000
get_info_comp<-function(x, trim=1, target_samp=10000, max_gens=10000000){
  untar(x, exdir=sub(".tar.gz", "",  x)) # unzip the folder
  unz_folder<-sub(".tar.gz", "",  x) # get the name of the unzipped folder
  #######################################
  ##### Read in the chains
  ...
  if(length(logfile)>0){
    ...
    ## loop to get the acceptance rates from each chain
    lines_acc<-list()
    for(i in 1:nruns){
      lines_start<-grep(paste0('Acceptance rates for the moves in the "cold" chain of run ', i, ":"), log, value=FALSE, ignore.case=TRUE)+2  # Line starting the table for chain 1 - +2 is because there is this line searched for that designated the chain, then a header, THEN the table starts
      line_end<-min(blank_lines[blank_lines>lines_start])-1  # end line is the first line of white space only past this, -1
      lines_acc[[i]]<-lines_start:line_end # Add these lines into the list of lines containing acceptance rates for each chain
    }
    acc_rates<-lapply(lines_acc, get.acc.rates, log=log)
    acceptances<-do.call(rbind, acc_rates)
